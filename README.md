...
- **知识积累**：每次修复后将失败模式写入 `ci-data` 分支，后续分析自动参考历史案例
...
│   │   ├── ci_data.py              # ci-data 分支读写（分析报告 + 知识库）
...
│   └── sync_poll_interval.py       # 同步 cron 表达式工具脚本
└── requirements.txt
```
...
| Secret | 用途 | 说明 |
|--------|------|------|
| `DISPATCH_TOKEN` | 触发 repository_dispatch、读写 ci-data 分支、checkout workflow 仓库 | GitHub PAT，需 `repo` + `workflow` scope |
| `GITCODE_WATCH_TOKEN` | 读取 GitCode 仓库 PR / CI 日志 | GitCode PAT，需 `read_repository`、`read_api` |
| `GITCODE_WRITE_TOKEN` | 向 GitCode fork 推送代码、创建 PR、评论 | GitCode PAT，需 `write_repository`、`write_api` |
| `OPENAI_API_KEY` | AI API Key（`AI_RUNNER=opencode` 时必填） | 兼容 OpenAI 接口的 Key |
| `CLAUDE_CREDENTIALS_JSON` | Claude.ai OAuth 凭证（`AI_RUNNER=claude-code-account` 时必填） | 见下方说明 |
| `WRITE_TOKEN` | 向 GitHub 目标仓库推送代码（仅监控 GitHub 仓库时需要） | GitHub PAT，需 `contents:write`、`pull-requests:write` |
...
| `AI_TIMEOUT_MS` | AI 单次调用超时（毫秒） | `1800000`（30 分钟） |
| `OPENAI_BASE_URL` | 自定义 API 地址（OpenCode，可选） | 使用默认 |
...
OpenCode 兼容 OpenAI 接口，支持 DeepSeek、通义、GLM 等模型。

| 提供商 | `AI_MODEL` 示例 |
|--------|----------------|
| DeepSeek | `deepseek/deepseek-v4-pro` |
| 阿里通义 | `alibaba-cn/qwen-plus` |
| 智谱 GLM | `alibaba-cn/glm-5.1` |
...
- **GitHub Contents API**：ci-data 分支读写（分析报告 + 失败模式知识库）
