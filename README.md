...
Fix PR CI 结果
  ...
  ├─ ci_failed（< 3 次）─→ 重新进入 ci-log-analysis
  └─ ci_failed（≥ 3 次）─→ 关闭 Fix PR，通知人工介入 ⚠️
```
...
| **Fix PR 自管理** | Fix PR CI 再次失败时自动追加 commit 重试，无需创建新 PR；超过最大重试次数（默认 3 次）自动关闭并通知人工介入 |
...
无需手动操作。Fix PR 收到 `ci_failed` label 后，下次 Monitor 轮询时系统会对该 Fix PR 对应的**原始 PR** 重新发起 ci-log-analysis，AI 根据新的失败日志追加 commit 修复。超过 3 次后自动关闭 Fix PR 并评论通知人工介入。
...
| 文件 | 覆盖范围 | 用例数 |
|------|----------|--------|
| `test_ci_gitcode_api.py` | `_url_score`、`_find_jenkins_url_in_comments`（含混合 SUCCESS/FAILED 表格）、`get_latest_failed_run` 完整逻辑 | 32 |
| `test_fix_pr_body.py` | Fix PR 标题提取（软件名+版本）、正文结构、ci-data 读取 fallback | 20 |
| `test_process_pr_events.py` | 预发布版本检测（大小写/点分隔/软件名边界）、`fix:` 前缀跳过 | 43 |
